...
data_frames <- lapply(31:51, function(days_back) {
  query <- sprintf('{
    "size": 10000,
    "query": {
      "match": {
...
#colnames(dat)<-c("TipoTeste"   ,      "dataTeste",         "resultado"   ,      "estado_teste"   ,"testes","n_testes","dataNotificacao","estado","municipio","tipo_teste_limpo","resultado_teste_limpo" )
#require(table1)
...
testes<-subset(dat,select = c(n_testes,dataNotificacao,municipio,testes,tipo_exames,resultado_exames,dataTeste_limpa,concluido))
a<-str_detect(testes$tipo_exames,"ANTÍGENO")
a<-subset(testes,a)
...
b<-subset(a,a$resultado_exames=="detectavel")
...
k<-subset(junto,junto$dt_coleta>= (Sys.Date()-30))
